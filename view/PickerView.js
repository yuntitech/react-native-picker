...
class PickerView extends BaseComponent {
  ...
  constructor(props) {
    ...
    for (let i = 0; i < length; i++) {
      this.colorPath.push(
        new Animated.Value(i == this.props.selectedIndex + 2 ? 1 : 0)
      );
    }
    this.path = new Animated.Value(
      -props.itemHeight * this.props.selectedIndex
    );
    this.state = {
      list: list,
    ...
    this.onStartShouldSetPanResponder =
      this.onStartShouldSetPanResponder.bind(this);
    ...
    this.parentTopY =
      this.mScreenHeight - props.itemHeight * 5 - this.getSize(15);
    ...
    if (nextProps) {
      ...
      if (listChange || indexChange) {
        ...
        if (listChange) {
          ...
          for (let i = 0; i < length; i++) {
            this.colorPath.push(
              new Animated.Value(i == nextProps.selectedIndex + 2 ? 1 : 0)
            );
          }
        }
        nextState.list = list;
  ...
  onStartShouldSetPanResponder(evt, gestureState) {
    if (
      evt.nativeEvent.pageY < this.parentTopY ||
      evt.nativeEvent.pageY > this.parentBottomY
    ) {
      return false;
    } else {
  ...
  onPanResponderMove(evt, gestureState) {
    ...
    if (this._previousTop > 0) {
      this._previousTop = Math.min(
        this._previousTop,
        this.maxTop + this.props.itemHeight
      );
    } else {
      this._previousTop = Math.max(
        this._previousTop,
        this.maxBottom - this.props.itemHeight
      );
    }
  ...
  onPanResponderEnd(evt, gestureState) {
    ...
    if (actionTime < 300 && Math.abs(gestureState.vy) < 0.1) {
      let clickPosition = -(
        parseInt((gestureState.y0 - this.parentTopY) / this.props.itemHeight) -
        2
      );
      ...
      let toValue =
        this._previousTop + gestureState.vy * this.props.itemHeight * 2;
      let number = Math.round(toValue / this.props.itemHeight);
      toValue = this.props.itemHeight * number;
      if (toValue > 0) {
        toValue = Math.min(toValue, this.maxTop);
      ...
      Animated.decay(this.path, {
        ...
        if (this.path._value % this.props.itemHeight == 0) {
          ...
          if (this.pathListener) {
            ...
            this.pathListener = null;
            let toValue =
              Math.round(this.path._value / this.props.itemHeight) *
      ...
      this.pathListener = this.path.addListener((listener) => {
        if (listener.value < this.maxBottom && this.pathListener) {
          ...
          Animated.timing(this.path, {
            ...
            //onSeleted
            this.onSeleted(
              Math.abs(this.maxBottom / this.props.itemHeight - 2)
            );
          });
        } else if (
          listener.value > this.maxTop - this.props.itemHeight &&
          this.pathListener
        ) {
          this.path.removeListener(this.pathListener);
          this.pathListener = null;
    ...
    global.timer.push(
      setTimeout(() => {
        ...
        this.props.onPickerSelected &&
          this.props.onPickerSelected(this.state.list[selectedIndex]);
      }, 20)
    );
  }
        ...
        <Animated.Text
          style={{
            color: this.colorPath[index].interpolate({
              inputRange: [0, 1],
              outputRange: [
                this.props.itemTextColor,
                this.props.itemSelectedColor,
              ],
            }),
